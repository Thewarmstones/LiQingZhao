...
for filelist_name in os.listdir(input_folder):
    ...
    for i in tqdm(range(len(data))):
        ele = data[i]
        new_prompt = prompt.replace('{{title}}',ele['title']).replace('{{content}}',ele['content']).replace('{{comment}}',ele['comment']).replace('{{anno}}',ele['anno']).replace('{{conversations}}',str(ele['conversations']))
        try:
